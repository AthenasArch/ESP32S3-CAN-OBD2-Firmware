...
public:
    ...
    // --- Inicialização ---
    int begin();                    // default (500k, 11-bit)
    int begin(uint32_t baud);       // sobrecarga (específico)
    int beginAuto();                // auto detecção
    void end();

    // --- Status da conexão ---
    bool connected() const;         // conexão válida?
    uint32_t currentBaud() const;   // baud atual (0 = não conectado)
    bool isExtended() const;        // true = 29-bit
    ...
    bool readPid(uint8_t pid, float &out);   // unificado (11/29-bit)
    ...
    void setHeartbeatPid(uint8_t pid = ENGINE_RPM);
    void setHeartbeatInterval(uint32_t ms);
    void setBackoff(uint32_t initialMs, uint32_t maxMs);

    // --- Manutenção (FSM) ---
    void tick(); // chama periodicamente para heartbeat e redetecção
...
private:
    ...
    // Overloads de leitura
    float pidRead(uint8_t pid); // alto nível (11-bit, retorna float)
    int pidRead(uint8_t mode, uint8_t pid, void *data, int length); // baixo nível

    // 29-bit helpers
    bool startCan29bit(uint32_t baud);
    bool send_pid_29(uint8_t service, uint8_t pid);
    bool recv_pid_29(uint8_t expected_service_resp, uint8_t pid, uint8_t *outData, uint32_t timeout_ms);
    bool pidRead29(uint8_t pid, float &outVal);

    // FSM de auto-detecção / heartbeat
    bool detectOnce();
    bool heartbeatOnce();
    void closeBus();
...
private:
    ...
    Obd2AddrMode _addrMode;  // modo atual (11/29-bit)
    uint32_t _baud;          // baudrate atual
    ...
    uint32_t _lastHbMs;
    uint32_t _lastDetectMs;
    uint32_t _backoffMs;
    uint32_t _backoffMaxMs;
};
