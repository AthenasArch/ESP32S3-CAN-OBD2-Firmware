                         ...
                         _lastPidResponseMillis(0)
{
    ...
    _hbPid = ENGINE_RPM;  // NOVO
    _hbIntervalMs = 1000; // NOVO (1s)
    _lastHbMs = 0;        // NOVO
    _lastDetectMs = 0;    // NOVO
    _backoffMs = 1000;    // NOVO (1s)
    _backoffMaxMs = 8000; // NOVO (8s)
}
...
int OBD2Class::begin()
{
    return begin(CAN_BPS_500K); // default
}

/// @brief
/// @param baud
/// @return
int OBD2Class::begin(uint32_t baud)
{
    if (!CAN0.begin(baud))
        return 0;
    ...
    // sucesso 11-bit
    _addrMode = OBD2_ADDR_11BIT; // NOVO
    _baud = baud;                // NOVO
    _connected = true;           // NOVO
    return 1;
}

bool OBD2Class::detectOnce()
{
    struct Try
    {
        uint32_t baud;
        ...
        {CAN_BPS_500K, OBD2_ADDR_11BIT, "11b/500k"},
        {CAN_BPS_250K, OBD2_ADDR_11BIT, "11b/250k"},
        {CAN_BPS_500K, OBD2_ADDR_29BIT, "29b/500k"},
        {CAN_BPS_250K, OBD2_ADDR_29BIT, "29b/250k"},
    };

    for (auto &t : tries)
    {
        bool started = (t.mode == OBD2_ADDR_11BIT) ? (begin(t.baud) == 1)
                                                   : startCan29bit(t.baud);
        if (!started)
            continue;

        delay(300); // estabiliza barramento

        float v;
        bool ok1 = readPid(PIDS_SUPPORT_01_20, v) || readPid(ENGINE_RPM, v);
        bool ok2 = ok1 && readPid(ENGINE_RPM, v);
        ...
        // falhou validação -> fecha e tenta próximo
        closeBus();
    }
    return false;
}

bool OBD2Class::heartbeatOnce()
{
    float v;
    if (readPid(_hbPid, v))
        return true;
    // fallback: só checar se responde algo
    return readPid(PIDS_SUPPORT_01_20, v);
}

void OBD2Class::setHeartbeatPid(uint8_t pid) { _hbPid = pid; }
void OBD2Class::setHeartbeatInterval(uint32_t ms) { _hbIntervalMs = ms; }
void OBD2Class::setBackoff(uint32_t initialMs, uint32_t maxMs)
{
    _backoffMs = initialMs ? initialMs : 1000;
...
int OBD2Class::beginAuto()
{
    struct Try
    {
        ...
        {CAN_BPS_500K, OBD2_ADDR_11BIT},
        {CAN_BPS_250K, OBD2_ADDR_11BIT},
        {CAN_BPS_500K, OBD2_ADDR_29BIT},
        {CAN_BPS_250K, OBD2_ADDR_29BIT}};

    for (auto &t : tries)
    {
        if (t.mode == OBD2_ADDR_11BIT)
        {
            if (begin(t.baud))
            {
                _addrMode = OBD2_ADDR_11BIT;
        ...
        {
            if (startCan29bit(t.baud))
            {
                _addrMode = OBD2_ADDR_29BIT;
                _baud = t.baud;
                return 1;
            }
        }
    }
    return 0; // nenhum protocolo encontrado
}
...
bool OBD2Class::readPid(uint8_t pid, float &out) {
    if (_addrMode == OBD2_ADDR_29BIT) {
        // 29-bit manual
        return pidRead29(pid, out);
    } else {
        // 11-bit alto nível
        if (!pidSupported(pid)) return false;
        float v = pidRead(pid); // float
...
bool OBD2Class::startCan29bit(uint32_t baud)
{
    if (!CAN0.begin(baud))
        return false;
    CAN0.watchFor();             // aceitar todas respostas
    _addrMode = OBD2_ADDR_29BIT; // NOVO
    _baud = baud;                // NOVO
    // _connected será setado após validação numa leitura
    return true;
}

bool OBD2Class::send_pid_29(uint8_t service, uint8_t pid)
{
    CAN_FRAME tx;
    tx.id = 0x18DB33F1; // Functional request
    tx.extended = true;
    tx.rtr = 0;
    tx.length = 8;
    tx.data.uint8[0] = 0x02;
    tx.data.uint8[1] = service;
    tx.data.uint8[2] = pid;
    for (int i = 3; i < 8; i++)
        tx.data.uint8[i] = 0x00;
    return CAN0.sendFrame(tx);
}

bool OBD2Class::recv_pid_29(uint8_t expected_service_resp, uint8_t pid, uint8_t *outData, uint32_t timeout_ms)
{
    ...
    while ((millis() - start) < timeout_ms)
    {
        ...
        if (CAN0.read(rx) != 0)
        {
            ...
            if (rx.length >= 3)
            {
                uint8_t svc = rx.data.uint8[1];
                uint8_t pid_rx = rx.data.uint8[2];
                if (svc == expected_service_resp && pid_rx == pid)
                {
                    for (int i = 0; i < rx.length; i++)
                    {
                        outData[i] = rx.data.uint8[i];
                    }
                    return true;
                }
            }
